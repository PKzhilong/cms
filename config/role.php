<?php

return [
    'system' => '系统模块',

    'system.index' => '系统首页',
    'system/index' => '首页',

    'system.config' => '系统配置',
    'system/config' => '设置',
    'system.update-cache' => '系统缓存',
    'system/update-cache' => '更新缓存',

    'system.admin' => '管理员管理',
    'system/admin' => '列表',
    'system/admin/create' => '新增',
    'system/admin/edit' => '修改',
    'system/admin/destroy' => '删除',
    'system/admin/modify' => '字段修改',
    'system/admin/password' => '密码重置',

    'system.role' => '角色管理',
    'system/role' => '列表',
    'system/role/create' => '新增',
    'system/role/edit' => '修改',
    'system/role/destroy' => '删除',
    'system/role/auth' => '授权',

    'system.menu' => '菜单管理',
    'system/menu' => '列表',
    'system/menu/create' => '新增',
    'system/menu/edit' => '修改',
    'system/menu/destroy' => '删除',
    'system/menu/config' => '配置',

    'system.upload' => '上传接口',
    'system/upload' => '图片上传',

    'system.dashboard' => '控制台',
    'system/dashboard' => '控制台',

    'system.addon' => '插件管理',
    'system/addon' => '插件列表',
    'system/addon/install' => '插件安装',
    'system/addon/uninstall' => '插件卸载',
    'system/addon/init' => '插件启用',
    'system/addon/modify' => '菜单显示',

    'system.attr' => '系统属性',
    'system/attr' => '属性列表',
    'system/attr/create' => '新增属性',
    'system/attr/edit' => '修改属性',
    'system/attr/destroy' => '删除属性',

    'system.diy-page' => '自定义页面',
    'system/diy-page' => '页面列表',
    'system/diy-page/create' => '新增页面',
    'system/diy-page/edit' => '修改页面',
    'system/diy-page/destroy' => '删除页面',

    'addon' => '插件模块',

    'user' => '会员模块',
    'user.admin' => '会员管理',
    'user/admin' => '会员列表',
    'user/admin/create' => '添加会员',
    'user/admin/edit' => '编辑会员',
    'user/admin/password' => '变更密码',
    'user/admin/account' => '资产账户',
    'user/admin/modify' => '字段修改',
    'user/admin/destroy' => '用户删除',
    'user/admin/balance' => '余额明细',
    'user/admin/point' => '积分明细',

    'user/admin/rank' => '会员等级',
    'user/admin/rank/create' => '添加等级',
    'user/admin/rank/edit' => '编辑等级',
    'user/admin/rank/destroy' => '删除等级',


    'article' => 'CMS模块',
    'article.admin' => '文章管理',
    'article/admin' => '文章列表',
    'article/admin/create' => '新建文章',
    'article/admin/edit' => '编辑文章',
    'article/admin/tags' => '设置标签',
    'article/admin/destroy' => '删除文章',
    'article/admin/category' => '文章分类',
    'article/admin/category/create' => '创建分类',
    'article/admin/category/edit' => '编辑分类',
    'article/admin/category/destroy' => '删除分类',
    'article/admin/category/metaToArticle' => '查询分类拓展',

    'article/admin/tag' => '文章标签',
    'article/admin/tag/create' => '创建标签',
    'article/admin/tag/edit' => '编辑标签',
    'article/admin/tag/destroy' => '删除标签',

    'article/admin/comment' => '文章评论',
    'article/admin/comment/config' => '评论设置',
    'article/admin/comment/destroy' => '删除评论',
    'article/admin/comment/modify' => '状态修改',


    'shop' => '商城模块',
    'shop.admin' => '商城后台',
    'shop/admin/category' => '商品分类',
    'shop/admin/category/create' => '创建分类',
    'shop/admin/category/edit' => '编辑分类',
    'shop/admin/category/destroy' => '删除分类',
    'shop/admin/category/metaToGoods' => '查询分类拓展',

    'shop/admin/goods' => '商品列表',
    'shop/admin/goods/create' => '创建商品',
    'shop/admin/goods/edit' => '编辑商品',
    'shop/admin/goods/destroy' => '删除商品',
    'shop/admin/pay/logs' => '支付流水',
];
